...
### Bước 2: Nạp dữ liệu cơ sở & Cấu hình bảo mật
...
Tiếp theo, tạo file `.env` từ file mẫu `.env.example` và điền cấu hình mật khẩu Database cũng như GEMINI API Key vào đó:
```bash
cp .env.example .env
```
...
- ⚙️ **Phương Pháp Luận Xử Lý Dữ Liệu:** [`docs/data_processing_methodology.md`](docs/data_processing_methodology.md)
